...
CoreMapper::CoreMapping CoreMapper::genWlMap(const Layer &layer, const tensor_shape &tensor, bool wgtB)
{
	if (REF_IS_INSTANCE(layer, ConvLayer))
	{
		// Conv Layer...
		const ConvLayer &cl = static_cast<const ConvLayer &>(layer);
		ConvWl wl(cl.get_workload(), tensor.bk);
		wl.K = tensor.c;
		wl.H = tensor.h;
		wl.W = tensor.w;
		if (REF_IS_INSTANCE(layer, GroupConvLayer))
	...
	else if (REF_IS_INSTANCE(layer, LRLayer) || REF_IS_INSTANCE(layer, UpsampleLayer))
	{
		assert(!wgtB);
		// LR Layer...
		const LRLayer &lrl = static_cast<const LRLayer &>(layer);
		...
		wl.W = tensor.w;
		wl.update_op();
		access_t tot_op = wl.calc_op(tensor.bk);
...
CoreMapper::CoreMapping PolarInst::genMapping(const CoreMapper::ConvWl &wl)
{
	...
	for (size_t i = 0; i < sizeof(all_parts) / sizeof(Part); ++i)
	{
		...
		{
		case PartDim::C:
			pnum_c *= core.bus.aLen;
			break;
		case PartDim::K:
			pnum_k *= core.bus.aLen;
			break;
		case PartDim::H:
			pnum_h *= core.bus.aLen;
			break;
		case PartDim::W:
			pnum_w *= core.bus.aLen;
			break;
		case PartDim::B:
			pnum_b *= core.bus.aLen;
			break;
		default:
			assert(false);
		}
		...
		{
		case PartDim::C:
			pnum_c *= core.bus.oLen;
			break;
		case PartDim::K:
			pnum_k *= core.bus.oLen;
			break;
		case PartDim::H:
			pnum_h *= core.bus.oLen;
			break;
		case PartDim::W:
			pnum_w *= core.bus.oLen;
			break;
		case PartDim::B:
			pnum_b *= core.bus.oLen;
			break;
		default:
			assert(false);
		}
...
void PolarInst::getCost(const ConvWl &wl)
{
	...
	buf_energy = al1Read * core.al1.RCost + al1Write * core.al1.WCost + wl1Read * core.wl1.RCost + wl1Write * core.wl1.WCost + ol1Read * core.ol1.RCost + ol1Write * core.ol1.WCost + ol2Read * core.ol2.RCost + ol2Write * core.ol2.WCost;
	energy_t ubuf_energy =
		+ul3Read * core.ul3.RCost + ul3Write * core.ul3.WCost;
	if (hasAL2)
	{
	...
	if (hasKBSD)
		noc_energy += wBSDTot * core.bus.hopCost;
	if (hasWBSD)
		noc_energy += aBSDTot * core.bus.hopCost;
	...
	*/
	...
	if (util > 1 + 1e-6) {
		std::cerr << "Util: " << util << " " << tot_op << " " << comp_time << " " << TGroup << " " << core.mac_num << std::endl;
		assert(false);
	}
...
void EyerissInst::getCost(const CoreMapper::ConvWl &wl)
{
	...
	if (Kt % K1 == 1)
		al1_k -= k_reply - (wl.K % k_reply);
	al1_w = _al1_buf * al1_k;

	energy_t buf_energy = 0, noc_energy = 0, mac_energy = 0, ubuf_energy = 0;

	buf_energy = al1_w * core.al1.WCost + wl1_w * core.wl1.WCost + pl1_w * core.pl1.WCost + al1_r * core.al1.RCost + wl1_r * core.wl1.RCost + pl1_r * core.pl1.RCost;
	ubuf_energy = ul2_w * core.ul2.WCost + ul2_r * core.ul2.RCost;

	noc_energy = bus_aw * core.ibus.BusCost + bus_ww * core.wbus.BusCost + bus_pw * core.pbus.BusCost;
