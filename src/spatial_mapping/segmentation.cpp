...
void Segment_scheme::sa_change()
{
    ...
    static len_t cnt = 0;
    static auto rndint = [&](int l, int r)
    { return (int)(gen() % (r - l + 1) + l); };
...
bool Segment_scheme::build_SchTree(bool use_placement, SchNode *iter_res, bool base, bool calc_noc, bool fast_mode)
{
    ...
    if (iter_res)
    {
        ...
        if (use_placement)
            dynamic_cast<Cut *>(schtree)->searchInc(ltree, fast_mode, placement);
        else
            schtree->searchInc(ltree, fast_mode);
    }
    else
    {
        Cluster c(0, Cluster::xlen * Cluster::ylen);
...
Segment_scheme *DP_search(int iter_num, bool base, bool fast_mode)
{
    ...
    for (len_t i = segment_head; i < layer_num; ++i)
        iter->batch[i] = dp_batch[layer_num - 1];
    iter->build_LTree_by_segmentation();
...
#endif
    std::cout << std::endl
              << "Cost (before SPM search): " << std::endl
              << iter->schtree->get_cost() << std::endl;
    if (base)
    {
...
#if OUTPUT_DETAIL
    for (auto i : iter->get_order())
        std::cout << i << " ";
    std::cout << std::endl;
    FOR_BITSET(i, iter->get_border())
        std::cout << i << " ";
    std::cout << std::endl;
    for (auto i : iter->get_batch())
        std::cout << i << " ";
    std::cout << std::endl;
#endif
    ...
    std::cout << std::endl
              << "Cost (after SPM search): " << std::endl
              << iter->schtree_spm->get_cost() << std::endl;
    std::cout << std::endl
              << std::endl;
