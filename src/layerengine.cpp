...
LayerScheme FastLayerEngine::fillin(LNode* curNode, const Light_placement &place,bool calc_noc,bool base){
	...
	assert("Not implemented for FastLayerEngine" && false);
	return layerSch;
}

CoreMapper::MapCost FastLayerEngine::Core_tensor_explore(const Layer&layer, const tensor_shape& tensor, bool wgtB){
	CoreMapper::CoreMapping tileSch;
	tileSch = mapper->genWlMap(layer, tensor, wgtB);
	return tileSch.cost;
}

CoreMapper::MapCost StdLayerEngine::Core_tensor_explore(const Layer&layer, const tensor_shape& tensor, bool wgtB){
	CoreMapper::CoreMapping tileSch;
	tileSch = mapper->genWlMap(layer, tensor, wgtB);
	return tileSch.cost;
}

CoreMapper::CoreMapping FastLayerEngine::Core_tensor_explore_full(const Layer&layer, const tensor_shape& tensor, bool wgtB){
	CoreMapper::CoreMapping tileSch;
	tileSch = mapper->genWlMap(layer, tensor, wgtB);
	return tileSch;
}

CoreMapper::CoreMapping StdLayerEngine::Core_tensor_explore_full(const Layer&layer, const tensor_shape& tensor, bool wgtB){
	CoreMapper::CoreMapping tileSch;
	tileSch = mapper->genWlMap(layer, tensor, wgtB);
	return tileSch;
}
