...
void ConvLayer::Workload::init(){
	...
	//assert(sH<=R && sW<=S);
	if (!(tot_op>0 && sH>0 && sW>0)) {
		std::cout << tot_op << " " << sH << " " << sW << std::endl;
		assert(false);
	}
}
...
void ConvLayer::Workload::update_op(){
	tot_op = (access_t)C*(access_t)K*(access_t)R*(access_t)S*(access_t)H*(access_t)W;
	if (tot_op == 0) {
		std::cout << C << " " << K << " " << R << " " << S << " " << H << " " << W << std::endl;
		assert(false);
	}
}
...
// added by wangxuan
const TransposeLayer::dim* get_transpose_order(const TransposeLayer& tl) {
	return tl.wl.order;
}
...
FetchSch UpsampleLayer::set_fetch(const PartSch& partSch, vol_t size, len_t B, len_t wgt_B) const {
	/*
	// No need to fetch.
	(void) partSch;
	(void) size;
...
vol_t UpsampleLayer::ifm_part(fmap_range& ifm_range, const PartSch& part) const {
	(void)ifm_range;
	(void)part;
	assert(part.size() == part.K);
	return DIVCEIL(ifm_range.c.size(), part.K) * ifm_range.b.size() \
...
vol_t UpsampleLayer::wgt_part(fmap_range& wgt_range, const PartSch& part) const {
	(void)wgt_range;
	(void)part;
	assert(false); // Not implemented.
	return 0;
