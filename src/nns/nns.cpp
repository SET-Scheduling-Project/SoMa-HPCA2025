...
Network gen_network(string model_name){
	...
	for (auto layer_ir:total_ir.items()){
		...
		/*
		previous:
		1. Some Layer's Output
		   1.tensor/vector: add into prevs
		   2.scalar: NOT add into prevs, scalar from scratchpad, not using l2 buffer
		2. constant
		   1.tensor/vector: add into ext_data,vector's H & W=1
		   2.scalar: NOT add into prevs, scalar from instruction config, not using l2 buffer
		*/
		...
		for(int i=0; i<previous_layer.size();i++){
			...
			if(input_shape_list[i] == scalar_shape || previous_layer[i].find("weight")!=string::npos ){   // if prevs_layer is scalar, do NOT handle it
				continue;
			}
			else if(previous_layer[i].find("constant")==string::npos && previous_layer[i].find("input")==string::npos && previous_layer[i].find("cls")==string::npos){   // TODO add support for vector. Moreover, ext_data's batch is not the same（N,C）/（1,C）
				prevs.push_back(layer_map[cur_prev]);
			}
			else{   // constant(include input) tensor/vector, as ext_data
				InputData data(previous_layer[i], fmap_shape(input_shape_list[i][1], input_shape_list[i][2], input_shape_list[i][3]));	 // TODO require IR provide [N,C,H,W] and input_shape_list
				ext_data.push_back(data);    // TODO	
			}
		}
		bool is_Elt = false;   // TODO if mul is related, sum_sub = true
