...
private:
    // std::shared_ptr<const Layer> l;
    const Layer* l;
    const Bitset ifmPrevs;
    const Bitset wgtPrevs;
    const Bitset prevs;
    const Bitset extPrevs;
    const Bitset orderPrevs;
...
public:
    Node(const Layer* _l, const Bitset& _ifmPrevs, len_t _external_C, bwidth_t width = 0, const Bitset& _wgtPrevs = {}, const Bitset& _extPrevs = {}, const Bitset& _orderPrevs = {}, lid_t _id = -1);
    Node(const Node& n) = delete;
    Node(Node&& n) = default;
    const Layer& layer() const;
    const Layer* layer_ptr() const;
    const std::string& name() const;
    lid_t getid() const;
    const Bitset& getIfmPrevs() const;
    const Bitset& getWgtPrevs() const;
    const Bitset& getExtPrevs() const;
    const Bitset& getPrevs() const;
    const Bitset& getOrderPrevs() const;
    const Bitset& getOrderNexts() const;
    bool hasWgtPrevs() const;
    bool hasExtPrevs() const;
    bool hasOrderPrevs() const;
...
private:
    std::vector<InputData> ext_inputs;
...
public:
    ...
    lid_t add(const Layer* l, const layer_set& ifmPrevs = {}, bwidth_t width = 0, std::vector<InputData> ext_data = {}, const layer_set& wgtPrevs = {}, const layer_set& orderPrevs = {});
    const Node& getNode(lid_t id) const;
    const InputData& getInputData(lid_t id) const;
    const std::vector<InputData>& getExtInputs() const;
    ...
    bool topoSort(std::vector<lid_t>& sorted) const;
