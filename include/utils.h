...
extern vol_t core_buffer_read_bandwidth;
...
extern len_t getLCM(len_t a, len_t b);
...
struct tensor_shape {
	...
	tensor_shape(len_t bk, len_t _c, len_t _h, len_t _w = 0);
	bool operator==(const tensor_shape& other) const;
	void update_size();
	vol_t get_size() const;
	friend std::ostream& operator<<(std::ostream& os, const tensor_shape& shape);
};
...
struct fmap_range{
	struct dim_range{
		...
		dim_range combine(const dim_range& other) const;
	...
	fmap_range combine(const fmap_range& other) const;
	bool is_empty() const;
...
extern std::random_device::result_type _seed_here;
